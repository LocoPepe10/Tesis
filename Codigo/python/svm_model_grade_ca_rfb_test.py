...
SVM_model_grade_CA_RBF_test(coeff_in, CAI_out, number_coeff, coeff_new, CAI_out_new, kernel_in, var_NU, var_C, n_folds)

svm_kernel = svr(kernel(kernel_in))#set a kernel type
#range_sigma=2.^[4:1:5];                    %set a param sigma range
range_sigma = 2. ** 4
svm_kernel_sigma = param(svm_kernel, mstring('rbf'), range_sigma)#set a param sigma range on a kernel
#a3=param(svm_kernel,'nu',[0:0.1:1]);
svm_kernel_sigma.nu = var_NU#set a value of NU
svm_kernel_sigma.C = var_C#set a value of C
svm_kernel_sigma.optimizer = mstring('andre')

hyperparameters_grid = mcat([mstring('score=cv;score.folds='), int2str(n_folds)])
#set hyperparameters score cv and folds=n_folds

svm_kernel_cv = gridsel(svm_kernel_sigma, mcellarray([hyperparameters_grid]))
...
n = 0
CAI_out_new_nc = CAI_out_new(mslice[1:mitad])
CAI_out_new_hc = CAI_out_new(mslice[mitad + 1:mitad * 2])

#Valores nuevos
CAI_est_nc = CAI_est.X(mslice[1:mitad], 1)
CAI_est_hc = CAI_est.X(mslice[mitad + 1:mitad * 2], 1)
...
#Calculo de ROC
[k11, k21, A1] = make_roc(mcat([CAI_est_nc, CAI_est_hc])); print k11; print k21; print A1
[k12, k22, A2] = make_roc(mcat([CAI_out_new_nc, CAI_out_new_hc])); print k12; print k22; print A2

hold(mstring('on'))
plot(k21, k11, mstring('b'))
plot(k22, k12, mstring('r'))
...
corr_test = corr(CAI_est.X, data_test.Y); print corr_test

#error_train=ecm_norm(a.X==1,data_train.Y)
error_test = ecm_norm(CAI_est.X, data_test.Y); print error_test
pause()
