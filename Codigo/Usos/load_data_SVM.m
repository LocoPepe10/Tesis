function [data_train data_test]=load_data_SVM(file)
    ...
    for i=1+num_subj_train:num_subj_test+num_subj_train
        for j=1:num_ARI
            input_test(: ,count_test :count_test +length_in_test -1)=data_in.Res(i,j).h1(:,1:length_in_test);
            output_test(count_test :count_test +length_in_test -1)=ones(1,length_in_test)*(j-1);
